...
### Frontend (`frontend/`)
React (Vite) + TypeScript, per [ADR-0013](adr/0013-frontend-architecture-and-tooling.md); visual design tokens per [ADR-0014](adr/0014-visual-design-system.md), with its three web fonts self-hosted via the `@fontsource/*` packages (imported in `src/main.tsx`) rather than loaded from Google Fonts — see [ADR-0021](adr/0021-self-hosted-web-fonts.md). Routes so far: `/` (landing page), `/login` (two-step email+OTP form), `/start` (protected — greets the logged-in learner by name once one is set, else by email, links to `/learn` and `/profile`; a second placeholder nav tile for Prüfungssimulation still isn't interactive; shows an additional "Admin" link when `user.is_admin`), `/learn` (protected — a small header-area exam-variant dropdown, an aggregate "Gesamtfortschritt" tile that embeds a category pie chart, and a read-only per-topic "Lernstand" list that is always expanded here, see [ADR-0018](adr/0018-learning-progress-model-and-gelernt-streak-rule.md); picking a topic doesn't lead anywhere yet; the progress section is a shared `ProgressSummarySection` component, also used by `/profile`, where the list is collapsible instead), `/profile` (protected — the learner's self-service account page: edit optional first/last name and gender, switch `exam_variant`, view the same learning-progress summary as `/learn` (topic list collapsed by default), change the account's email address via a verify-with-code flow, see "Mitglied seit", and delete the account with a retype-your-email confirmation), and `/admin` (protected, additionally gated on `user.is_admin` — look a user up by email, export their data as JSON, or delete their account; see Admin / GDPR tooling below and [ADR-0019](adr/0019-admin-allowlist-and-manual-gdpr-fulfillment.md)). Zustand (`src/store/authStore.ts`) holds the current user and derives "logged in" from a `GET /auth/me` call, never from inspecting a token — the frontend never reads the session cookie directly (ADR-0012). `src/api/client.ts` is the thin typed `fetch` wrapper named in ADR-0013: always `credentials: "include"`, and a uniform 401 handler that clears the auth store. `src/routes/ProtectedRoute.tsx` redirects to `/login` when not authenticated.

The progress tile's pie chart (`ProgressPie.tsx`, hand-built SVG, no chart library) shows one slice per main category — Navigation, Schifffahrtsrecht, Wetterkunde, Seemannschaft, where `seemannschaft_allgemein` and the variant-specific `_segeln`/`_motor` subjects are merged into one. Slice size is the category's share of all questions; a radial gradient runs from strong (center) to faint (edge), with the strong zone reaching out to learned/total of the radius, so a fully learned slice is solid and one with nothing learned has no strong core. The pie sits small (with its legend beside it) to the right of the percentage in the progress tile; hovering a slice or legend row highlights it. The per-category sums are aggregated client-side from `/progress/summary`; no backend change.
...
`PATCH /api/v1/auth/me` is a partial update (`exclude_unset` semantics: an omitted field is untouched, an explicit `null` clears it) covering `exam_variant` (`"motor"` | `"segeln_und_motor"`, used by `/api/v1/questions` to default which subjects are returned — see Question Catalog in `CLAUDE.md`) and the optional, self-reported `first_name`/`last_name`/`gender` (`gender` validated against a fixed set the same non-`Literal` way as `exam_variant` — see `GenderField` in `backend/app/schemas/auth.py`). Called from `/learn` (exam variant only) and `/profile` (all four fields).

Changing the account's email is a separate, verify-before-commit flow rather than part of that `PATCH`: `POST /api/v1/auth/me/email/request` sends a confirmation code to the *new* address, reusing the login-OTP machinery (`otp_codes` table, `backend/app/core/otp.py`) but rejecting up front with `409` if that address already belongs to another account, and with `403` if `ALLOWED_EMAILS` is set and the address isn't on it (the change would otherwise lock the learner out: `otp/request` silently ignores that address, so they couldn't log in again once the session ends) — a deliberate deviation from `otp/request`'s anonymous anti-enumeration posture, since the caller here is already authenticated. `POST /api/v1/auth/me/email/verify` checks the code and commits the change, `409`-ing again if the address was claimed by someone else in the meantime (a request-time-check vs. commit-time race). The request endpoint is rate-limited two ways: per-IP (`backend/app/main.py`'s `rules`, same tightness as `otp/request`) and per-authenticated-user (`check_and_record` in `backend/app/core/rate_limit.py`, extracted from `RateLimitMiddleware` so both the IP-keyed middleware and this user-keyed application-level check share one primitive) — the per-IP cap alone wouldn't stop one account probing many target addresses from multiple IPs. The per-user check runs before the `403`/`409` checks, so a probe that finds a taken address uses up quota too. A wrong/expired code returns `400`, not `401`: the frontend's `apiClient` treats *any* `401` as "the session is dead" and force-logs-out the caller, which would be wrong here since the caller is already validly authenticated.

Every `otp_codes` row carries a `purpose` (`login` | `email_change`, `backend/app/core/otp.py`), and a code is only accepted by the flow it was issued for. Without it, an email-change confirmation code also worked on `otp/verify` — minting a login, and a new account, for an address `ALLOWED_EMAILS` never let through `otp/request`. The per-address cooldown and hourly cap are scoped to `(email, purpose)` as well, so email-change requests aimed at an address can't use up that address's login-code quota. Both flows share one pair of helpers in `backend/app/api/v1/auth.py` (`_issue_otp_code`, `_consume_otp_code`).

`DELETE /api/v1/auth/me` lets a learner delete their own account (the frontend confirms this by having them retype their own email, the same idiom as the admin GDPR delete tool below). The deletion logic itself (`question_progress` rows, any `otp_codes` rows for the address, then the `users` row) lives in `backend/app/services/user.py:delete_user_and_progress`, shared with `DELETE /api/v1/admin/users/{id}` below.
...
`/api/v1/admin/*` fulfills the DSGVO Art. 15/17/20 rights promised in the frontend's `/privacy` page: `POST /users/search` looks a user up by email, `GET /users/{id}/export` returns their data (every account field, including the optional name/gender, + `question_progress`, denormalized with subject/number — no question/answer text) as JSON, `DELETE /users/{id}` deletes the account via the same `delete_user_and_progress` helper `DELETE /api/v1/auth/me` uses (see Auth above) — `question_progress` rows first, explicitly, not relying on the DB `ondelete="CASCADE"`, which only fires reliably on Postgres, not on the SQLite engine the test suite uses. Gated by `require_admin` (`backend/app/core/jwt.py`): valid JWT, then the caller's email must be in the `ADMIN_EMAILS` allowlist (`Settings.admin_emails_set`) — unlike `ALLOWED_EMAILS`, empty/unset means *no* admins, not open access. `GET /api/v1/auth/me` also reports a computed `is_admin` boolean (same allowlist check), which the frontend's `/admin` page and the "Admin" link on `/start` use to gate access — see [ADR-0019](adr/0019-admin-allowlist-and-manual-gdpr-fulfillment.md) for why this is an email allowlist rather than a DB role, and manual admin-driven fulfillment rather than user self-service.
...
### CI/CD
GitHub Actions. `.github/workflows/backend-ci.yml`: separate `lint` (ruff), `test` (pytest on SQLite, 80% line+branch coverage gate), `migrations` (Alembic upgrade/check/downgrade against a Postgres 16 service), `integration-tests` (runs the hand-written Newman collection against a real uvicorn + Postgres, see External integration tests below), and `postman-collection` (regenerates the API-reference Postman collection from the live OpenAPI schema, fails if the committed one is stale) jobs. `.github/workflows/frontend-ci.yml`: `lint` (ESLint + Prettier check) and `test` (`tsc -b` + Vitest, same 80% line+branch coverage gate). Both run on every push to `main` and every PR.

### External integration tests
`postman/integration-tests.postman_collection.json` (see `CLAUDE.md` → Integration Tests) — a separate, hand-written Postman collection that black-box tests a real backend over HTTP: auth guards, CORS, security headers, the full OTP login round-trip via `GET /api/v1/auth/otp/_dev-peek` (dev/test-only, see [ADR-0011](adr/0011-dev-only-otp-peek-endpoint-for-external-integration-tests.md)), questions/topics/progress incl. exam-variant filtering, profile updates, the verify-before-commit email change and account self-deletion, the admin GDPR tools (with their non-admin 403s), and both rate limits. Runnable without a Python environment via `./scripts/run_integration_tests.sh` (Newman, over `npx`), and run in CI by `backend-ci.yml`'s `integration-tests` job against a real uvicorn + freshly migrated and seeded Postgres. Unlike the API-reference collection above, it isn't generated from the OpenAPI schema, so it's kept in sync by a tripwire instead: `backend/tests/test_integration_collection.py` fails the normal pytest run when a route has no request in the collection (or a request targets a removed route), and CI executing the collection catches assertions that no longer hold.
