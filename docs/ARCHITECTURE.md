...
4. **Lotsen-Check** ("Antwort vom Lotsen bewerten lassen", [ADR-0031](adr/0031-ai-answer-check-with-claude-haiku.md)): for accounts with `token_balance > 0`, Claude Haiku *suggests* a grade plus feedback (1 token per check, [ADR-0043](adr/0043-token-based-ai-grading-monetization.md)), and the learner still confirms. Limited to a weekly budget on top of the token balance (week starts Monday; the default is set on `/admin/settings`, overridable per account, [ADR-0036](adr/0036-weekly-ai-check-budget-with-admin-overrides.md)), hardened against prompt injection ([ADR-0040](adr/0040-ai-grading-sanitizer-and-abuse-monitoring.md)).
...
**Operator tools** (`/admin`, gated by the `ADMIN_EMAILS` allowlist, [ADR-0019](adr/0019-admin-allowlist-and-manual-gdpr-fulfillment.md)): browse and search the accounts (email or name), open one to export its data as JSON, delete it or set its flags and AI budget, look up question and official-answer texts across the catalog, read question reports and KPIs. The pages share one layout with a tab per section (`components/AdminLayout.tsx`). Data-subject requests are fulfilled by hand with these ([runbook](RUNBOOK.md#data-subject-requests-dsgvo)).
...
- **Prerendered public pages**: `npm run build` renders `/`, `/faq`, `/imprint`, `/privacy`, `/agb` and `/ablauf` to static HTML (`dist/index.html`, `faq.html`, …) so crawlers see its content; the client hydrates it. Each of these but `/` also gets its own `<title>`/description/OG/Twitter/canonical, generated in `prerender.mjs`. Every other route gets the empty shell (`dist/app.html`) via the SPA fallback ([ADR-0025](adr/0025-build-time-prerender-of-the-landing-page.md)).
- **Routing**: public pages (landing, login, legal) and protected pages (start, learn, practice, exam simulation, profile, admin) behind `ProtectedRoute`. The admin pages are nested routes under `AdminLayout`, which does the admin check once.
...
| `grading` | The Lotsen-Check (`POST /questions/{id}/ai-grade`): 1 token per check, per-question/day and per-hour caps, the persisted weekly budget on top of the token balance, a process-wide cap on concurrent LLM calls, the sanitizer | [0031](adr/0031-ai-answer-check-with-claude-haiku.md), [0036](adr/0036-weekly-ai-check-budget-with-admin-overrides.md), [0040](adr/0040-ai-grading-sanitizer-and-abuse-monitoring.md), [0043](adr/0043-token-based-ai-grading-monetization.md) |
...
| `admin` | User list with search (`GET /admin/users`), GDPR lookup/export/delete, question-text search over the cached catalog (`GET /admin/questions`), question reports, aggregate KPIs (`GET /admin/kpis`), weekly AI-check budget and every price/package (app-wide defaults in `app_settings` via `/admin/settings`), granting tokens/Werbefrei by hand (`PATCH /admin/users/{id}`), read-only AI-grading abuse signal (`ai_flags_count`), allowlist-gated | [0019](adr/0019-admin-allowlist-and-manual-gdpr-fulfillment.md), [0032](adr/0032-daily-kpi-report.md), [0036](adr/0036-weekly-ai-check-budget-with-admin-overrides.md), [0040](adr/0040-ai-grading-sanitizer-and-abuse-monitoring.md), [0043](adr/0043-token-based-ai-grading-monetization.md) |
...
| `users` | Account and profile, `ads_removed` and the token balance ([ADR-0043](adr/0043-token-based-ai-grading-monetization.md)), this week's AI-check counter and override, the sanitizer flag count, AGB acceptance version/timestamp, last-login timestamp ([ADR-0041](adr/0041-agb-acceptance-and-inactivity-retention.md)) | Auth and admin flows, the AI check |
...
| `app_settings` | Operator-tuned app-wide values (the default weekly AI-check budget, every token-package/Werbefrei price) | `/admin/settings` ([ADR-0036](adr/0036-weekly-ai-check-budget-with-admin-overrides.md), [ADR-0043](adr/0043-token-based-ai-grading-monetization.md)) |
