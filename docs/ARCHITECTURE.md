...
9. **Feedback** ([ADR-0030](adr/0030-question-reports-and-feedback-channels.md)): a "Feedback" `mailto:` link, "Frage melden" under every question (counted in the daily KPI report; the comments are read via the Render Shell), and coarse Umami funnel events (fixed keys only, never free text).
