...
**Operator tools** (`/admin`, gated by the `ADMIN_EMAILS` allowlist, [ADR-0019](adr/0019-admin-allowlist-and-manual-gdpr-fulfillment.md)): browse and search the accounts (email or name), open one to export its data as JSON, delete it or credit tokens/Werbefrei by hand, look up question and official-answer texts across the catalog, read question reports and KPIs. The pages share one layout with a tab per section (`components/AdminLayout.tsx`). Data-subject requests are fulfilled by hand with these ([runbook](RUNBOOK.md#data-subject-requests-dsgvo)).
...
| `admin` | User list with search (`GET /admin/users`), GDPR lookup/export/delete, question-text search over the cached catalog (`GET /admin/questions`), question reports, aggregate KPIs (`GET /admin/kpis`), every price/package (app-wide defaults in `app_settings` via `/admin/settings`), granting tokens/Werbefrei by hand (`PATCH /admin/users/{id}`), read-only AI-grading abuse signal (`ai_flags_count`), allowlist-gated | [0019](adr/0019-admin-allowlist-and-manual-gdpr-fulfillment.md), [0032](adr/0032-daily-kpi-report.md), [0040](adr/0040-ai-grading-sanitizer-and-abuse-monitoring.md), [0043](adr/0043-token-based-ai-grading-monetization.md) |
