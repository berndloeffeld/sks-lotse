...
| `admin` | GDPR lookup/export/delete, question reports, aggregate KPIs (`GET /admin/kpis`), weekly AI-check budget (per-user override, app-wide default in `app_settings` via `/admin/settings`), allowlist-gated | [0019](adr/0019-admin-allowlist-and-manual-gdpr-fulfillment.md), [0032](adr/0032-daily-kpi-report.md), [0036](adr/0036-weekly-ai-check-budget-with-admin-overrides.md) |
