...
- **Ads**: the `adsense-snippet` plugin in `vite.config.ts` puts Google's AdSense script into the built HTML `<head>` only when `VITE_ADSENSE_CLIENT_ID` is set, and only on the prerendered public pages. `prerender.mjs` strips it from `app.html`, and `routes/AdFreeDocument.tsx` reloads `/login` and every logged-in route out of a document that still has it ([ADR-0041](adr/0041-no-ad-script-in-the-logged-in-app.md)). `src/ads.ts` holds the runtime helpers. Consent comes from Google's own TCF consent management, re-openable via the footer's "Cookie-Einstellungen" from any page ([ADR-0027](adr/0027-adsense-with-google-consent-management.md)). No ad units are rendered yet.
