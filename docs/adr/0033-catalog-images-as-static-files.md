...
- The frontend's `QuestionImages` shows them below the question text and inside "Amtliche Antwort", at twice the pixel size (the catalog's images are ~72 dpi line art), on a white background in dark mode, with width/height set so the layout doesn't jump.
