...
The same-site assumption above briefly looked broken during rollout — a Render Custom Domain cap almost forced the frontend and API onto genuinely different registrable domains — but [ADR-0016](0016-third-paid-custom-domain.md) resolved that by paying for a third domain slot rather than accepting a cross-site cookie. Same-site holds, unchanged.
