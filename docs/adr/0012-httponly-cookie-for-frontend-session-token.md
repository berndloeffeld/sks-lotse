...
The same-site assumption above didn't hold once the frontend's actual deployment was decided — a Render account-wide Custom Domain cap made the planned `api.sks-lotse.de` subdomain impractical (see [ADR-0015](0015-frontend-deployment-topology.md)'s addendum), so the frontend and API ended up on genuinely different registrable domains. [ADR-0016](0016-cross-origin-session-cookie.md) covers what changed: `SameSite=None` (`Secure` now unconditional, not just in production) and the CSRF reasoning re-examined for that. The core decision here — httpOnly cookie over `localStorage` — is unchanged.
