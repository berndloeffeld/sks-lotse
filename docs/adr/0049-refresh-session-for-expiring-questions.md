...
- **Selection:** a random sample of `REFRESH_SESSION_SIZE` (20) from the candidates, `ORDER BY random() LIMIT 20` (`GET /progress/refresh/questions`). Random rather than most-urgent-first, so repeated sessions don't always show the same questions, and the session stays short however many candidates exist.
...
- Random selection means the sample can miss the most overdue question; if that turns out to matter, order by `review_due_at` instead.
