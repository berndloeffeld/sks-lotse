...
- The classification and merge steps are the first real use of `OPENAI_API_KEY` in this codebase (ahead of the actual grading feature) — `openai` is now a `requirements.txt` dependency. Both are one-off/re-runnable dev scripts, not request-path code; the review-before-apply YAML files are committed, so `apply` never needs to hit the LLM again unless the catalog itself is re-imported.
