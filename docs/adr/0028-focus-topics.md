...
- **Fokus session** (`/learn/fokus`, started from the Fokus band): one run over the open questions of *all* Fokus topics, independent of topic. `GET /progress/focus/questions` returns those that are not gelernt, ordered by `question_progress.last_correct_at` ascending — questions never answered at all first, then those never answered "Richtig", then the one whose last correct answer is longest ago. Only "Richtig" sets `last_correct_at`, so a later "Falsch" doesn't make a question look recently mastered. The list is fetched once per session and played in that order (no re-ordering after each answer); the frontend reuses the practice loop with the order kept and the topic named in the question header.
...
- Rejected: only *hiding* a completed topic (deriving it on read). Then a later decay or setback would silently bring the topic back into the focus, which surprises more than it helps — the learner can simply mark it again.
