...
Status: Accepted — the "separate from the Lernstand" bullet is superseded by [ADR-0037](0037-exam-richtig-answers-feed-the-lernstand.md).
...
- **Flow**: answer everything (autosaved), submit, then self-assess question by question with the official answer shown. Official answers are withheld from the API until the exam is submitted. One running exam per learner (`409` otherwise), enforced by a partial unique index on `exam_attempts(user_id) WHERE submitted_at IS NULL`, so two parallel starts can't both succeed. After the last self-assessment the attempt is complete and final.
...
- **Separate from the Lernstand**: exams don't touch `question_progress`, so a good exam result doesn't mark questions "gelernt" and practice isn't distorted by test runs. *(Superseded by [ADR-0037](0037-exam-richtig-answers-feed-the-lernstand.md): correct answers now count.)*
