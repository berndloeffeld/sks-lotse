...
| What the product can do today, for product, sales and sailing instructors (German) | [docs/FEATURES.md](docs/FEATURES.md) |
...
Frontend enforces **95% lines / 90% branches** via Vitest's built-in coverage (`frontend/vite.config.ts`, `test.coverage.thresholds`; actual ~98% / ~94%), run with `npx vitest run --coverage`. `.github/workflows/frontend-ci.yml`'s `test` job runs `tsc -b` plus that command on every push to `main` and every PR — a required check, like the backend's `test`.
...
- **`STRIPE_CHECKOUT`** — `off` (default), `admins` or `on`; anything else fails at startup. `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` are `sync: false` secrets, `STRIPE_PRODUCT_TOKENS_S|M|L|XL` the Stripe product ids (different in test and live). Not set = nobody can buy ([ADR-0048](docs/adr/0048-stripe-hosted-checkout-with-webhook-fulfilment.md), [docs/stripe/README.md](docs/stripe/README.md)).
