...
### Deployment (Render)
Provisioned as code via `render.yaml` (repo root) — see [docs/adr/0005-render-deployment-topology.md](docs/adr/0005-render-deployment-topology.md) for the backend/DB reasoning and [ADR-0016](docs/adr/0016-cross-origin-session-cookie.md) for the frontend (supersedes [ADR-0015](docs/adr/0015-frontend-deployment-topology.md)'s original plan). Two web services (backend, Python; frontend, static site) + one managed Postgres, Frankfurt region, production only (no staging yet). `sks-lotse.de`/`www.sks-lotse.de` → frontend; `sks-lotse.com` stays on the backend, unchanged, still 301s to `.de`; the frontend calls the backend at its default `https://sks-lotse-backend.onrender.com` URL (`VITE_API_BASE_URL`), not a custom API subdomain. **The Render account's plan caps Custom Domains at 2, total for the whole account** (not per service — `www` is bundled into its root domain's one slot, not counted separately) — `sks-lotse.de` and `sks-lotse.com` already spend both. Keep this in mind before adding any further Custom Domain anywhere in the account; see ADR-0016 for what this ruled out.
...
- The API has **no custom domain** — the frontend calls the backend at its default `https://sks-lotse-backend.onrender.com` URL instead (`VITE_API_BASE_URL`). See [ADR-0016](docs/adr/0016-cross-origin-session-cookie.md): the account's 2-domain-total cap is already spent on `sks-lotse.de` and `sks-lotse.com`, so a dedicated `api.sks-lotse.de` (ADR-0015's original plan) isn't available without dropping one of those or upgrading the plan.
