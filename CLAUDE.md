...
6. Learners can mark topics as **Fokus** (star on `/learn`); the Fokus band shows how many of those questions are sicher gelernt (half-life model, next bullet) or teilweise gelernt. A Fokus topic drops out permanently once all its questions are learned ([ADR-0028](docs/adr/0028-focus-topics.md)). Focus marks are deleted with the account and part of the admin DSGVO export.
