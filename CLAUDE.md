...
- Complexity is capped via `C901`/`PLR0912` (`max-complexity`/`max-branches` = 15, just above today's worst function) — lower the ceiling as offenders get refactored. `PLR0913` (argument count) is deliberately off: FastAPI routes take their dependencies as arguments.
