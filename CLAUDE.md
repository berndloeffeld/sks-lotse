...
- **The catalog seeds itself automatically.** `backend/app/services/catalog_seed.py` (`seed_catalog()`) is the single source of truth for PDF → `questions`/`topics` rows, and it runs as an Alembic **data migration** (`alembic/versions/16af6f481bf6_seed_question_catalog.py`) — meaning every `alembic upgrade head` populates/rebuilds the full catalog, including Render's `startCommand` before every deploy. No one has to remember to run a script against production; the schema migration and the data that belongs with it land together. Downgrading that migration deletes all `questions`/`topics` rows.
- Getting there is a 3-stage pipeline, but only the first and third stages ever touch an external service, and both only from a developer's machine — the migration itself calls nothing but the DB:
  1. **Parse**: `backend/scripts/import_catalog.py` (or the migration) parses the PDF into raw `questions` rows (still `seemannschaft_1`/`seemannschaft_2` at this point). Known parsing limitations are listed in its docstring.
  2. **Merge**: `backend/scripts/merge_seemannschaft.py propose` computes candidate duplicate pairs by plain text-similarity (no LLM) into `scripts/data/seemannschaft_duplicates.yaml` for a human to review by hand; `apply` (or the migration) then reads that reviewed file and collapses `seemannschaft_1`/`seemannschaft_2` into the 3 subjects above.
  3. **Classify**: `backend/scripts/manage_topics.py propose` calls the Anthropic API (`settings.anthropic_api_key`, a local dev-tooling-only credential, separate from `OPENAI_API_KEY`/the grading feature below) to classify questions into the fixed names from `topics.yaml`, writing `scripts/data/topic_assignments/<subject>.yaml` for a human to review by hand; `apply` (or the migration) then applies that reviewed file to `Question.topic_id`. The LLM only ever classifies into existing names — it never invents or renames a topic.
  - Only the *review artifacts* (`seemannschaft_duplicates.yaml`, `topic_assignments/*.yaml`) need to exist before `alembic upgrade head` can seed a new environment — they're committed to the repo, so this is already true everywhere. Re-running `propose` (and re-reviewing) is only needed if the source PDF or the topic taxonomy itself changes; a new migration would then apply the updated result.
...
- Current phase: Phase 1 — backend foundation, wrapping up. Done: catalog import (incl. the official topic taxonomy and Seemannschaft I/II merge, [ADR-0017](docs/adr/0017-official-topic-taxonomy-and-seemannschaft-merge.md)), email+OTP login with JWT sessions (now cookie-based for the frontend, [ADR-0012](docs/adr/0012-httponly-cookie-for-frontend-session-token.md)), deployment to Render with CI/security tooling (backend and frontend both live), a frontend scaffold covering landing/login/start/learn/admin plus the legal pages (per ADR-0013/0014), a learning-progress data model with a read-only Lernstand overview ([ADR-0018](docs/adr/0018-learning-progress-model-and-gelernt-streak-rule.md)), and GDPR admin tooling ([ADR-0019](docs/adr/0019-admin-allowlist-and-manual-gdpr-fulfillment.md)). Next: core grading flow (OpenAI) and the question list/answering UI it unblocks.
