...
- **Rebuilding the catalog is a 3-step, order-dependent pipeline** (each step re-runnable, each documented in its own docstring):
  1. `backend/scripts/import_catalog.py` — PDF → raw `questions` rows (still `seemannschaft_1`/`seemannschaft_2` at this point), replaces the table's contents. Known parsing limitations are listed in its docstring.
  2. `backend/scripts/merge_seemannschaft.py propose` then (after reviewing `scripts/data/seemannschaft_duplicates.yaml` by hand) `apply` — collapses `seemannschaft_1`/`seemannschaft_2` into the 3 subjects above. Matching is plain text-similarity (no LLM needed — the duplicates are near-identical text).
  3. `backend/scripts/manage_topics.py propose` (calls the OpenAI API — the first real use of `settings.openai_api_key` in this codebase, ahead of the grading feature) then, after reviewing `scripts/data/topic_assignments/<subject>.yaml` by hand, `apply` — sets `Question.topic_id`. The LLM only ever classifies questions into the fixed names from `topics.yaml`; it never invents or renames a topic.
  - Because step 1 always recreates its raw output from scratch, steps 2 and 3 must be re-run after every catalog re-import — they're never a no-op on stale data, they'd just silently do nothing (no `seemannschaft_1`/`seemannschaft_2` rows left to match, no rows in the new subjects to classify).
...
- **Planned, not yet read by the app:** `GOOGLE_OAUTH_CLIENT_ID`/`_SECRET`, `FACEBOOK_OAUTH_CLIENT_ID`/`_SECRET`, `X_OAUTH_CLIENT_ID`/`_SECRET` (SSO isn't built). `ADSENSE_CLIENT_ID` is already in `Settings`/`render.yaml` but unused until ads land. `OPENAI_API_KEY` is used locally by `backend/scripts/manage_topics.py` (see Question Catalog) but not yet by the running app itself — that's still the grading feature, not yet built.
